...
local function run(msg, matches)
  ...
  if data[tostring(msg.to.id)]['settings']['anti_username'] then
    if data[tostring(msg.to.id)]['settings']['anti_username'] == 'yes' then
      if antiusername[msg.from.id] == true then 
        return
      end
      send_large_msg("chat#id".. msg.to.id , "username is not allowed here")
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] kicked (Successful Arisharr) ")
